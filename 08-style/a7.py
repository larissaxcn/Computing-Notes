def champions(ledger, threshold, num_champs):
    ...
    # Loop over ledger, tally points for each house recorded in the ledger and
    # add house to finishers list if they surpass the required threshold
    for house, points in ledger:
        # Get current score for house (0 if no current score)
        current_score = tally.get(house, 0)
        if current_score < threshold:
            # Add new points  in ledger to existing point tally for house if
            # under threshold; once over threshold, no need to compute
            tally[house] = current_score + points
            if tally[house] >= threshold:
                # if tally surpasses threshold, add to finishers list and
                # break from loop if reach limit for number of finishers
