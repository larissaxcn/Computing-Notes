...
GitHub Copilot and the following information. May provide more detailed prompt,
iterative prompting as well.
...
def lowest_temperature_alt_1(data, date):
    ...
    note that this particular function will fail on tests 1 and 2 because it
    will not adjust "city" if the lowest temperature appears first in the data]
    
    Also fails on test 4 where first row's temp is lower than low on `date`
...
def lowest_temperature_alt_2(data, date):
    '''
    [Generated with GitHub Copilot using function signature and docstring; 
    this one is pretty inefficient with multiple loops through the data;
...
def lowest_temperature_alt_3(data, date):
    '''
    [Generated with GitHub Copilot using function signature and docstring; 
    passes tests 1-3, but fails test 4 when data is different size from
    temperatures list. Also, indexing and min ops are expensive after already
    looping through full dataset]
